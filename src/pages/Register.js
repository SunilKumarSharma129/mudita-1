  ...
  const handleSignUp = useCallback(
    async (event) => {
      ...
      try {
        ...
        console.log("Registered with Us");
        setLoading(true);
        await createUserWithEmailAndPassword(auth, email.value, password.value);
        setLoading(false);
        console.log("Registered with Firebase");
        navigate("/Login");
        // history.push("/");
      } catch (error) {
        alert(error);
            ...
            <div className="flex flex-col gap-4 mb-2">
              <div className="flex flex-col gap-4 mb-1">
                <div className="relative">
                  <input
                    type="text"
                    className="flex-1 w-72 px-4 py-2 text-base text-gray-700 placeholder-gray-400 bg-white border border-transparent border-gray-300 rounded-lg shadow-sm appearance-none focus:outline-none focus:ring-2 focus:ring-custom-green focus:border-transparent"
                    name="fullName"
